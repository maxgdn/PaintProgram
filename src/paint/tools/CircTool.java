...
public class CircTool implements Tool {
    ...
    public void leftClick(MouseEvent event) {
        ...
        if (!point1Set) {
            ...
            if (MainFrame.getInstance().getToolPanel().getFillIsTrue()) {
                graphics.fillOval(leftPoint, topPoint, rightPoint - leftPoint, bottomPoint - topPoint);
            } else {
                graphics.drawOval(leftPoint, topPoint, rightPoint - leftPoint, bottomPoint - topPoint);
            }
            point1Set = false;
        }
    }
